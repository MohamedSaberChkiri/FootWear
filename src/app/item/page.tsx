export default function ItemInfo(
    props :{
        productName : string,
        price: number ,
        manufactor : string,
        currentStock : number,
        productDescription : string ,
        details : string,
        reviews : number
    }
){

    const backGround = 'url("https://static.nike.com/a/images/t_PDP_1728_v1/f_auto,q_auto:eco/b01c67f2-2481-45d7-b383-a1476d768f6e/air-force-1-07-damenschuh-dgr2tk.png")'
    return(
        <div>
            
            <div className="flex flex-wrap items-center justify-center my-[3vh] gap-4">
                <div className="w-[40rem] h-[40rem] border" style={{ background:  backGround, backgroundPosition: "center", backgroundSize: 'cover' }}></div>
                <div className="w-[40rem] h-[40rem] flex flex-col items-start justify-around border">

                </div>
            </div>



            <div></div>


        </div>
    )
}